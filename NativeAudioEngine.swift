...
final class NativeAudioEngine {
 ...
 var pan:Double=0 {
  didSet { DtSoundPlayer.shared.setPan(pan) }
 }
 ...
 func start(){
  ...
  let renderBlock: AVAudioSourceNodeRenderBlock = { [weak self] _, _, frames, audioBufferList -> OSStatus in
   ...
   self.lock.lock()
   var localLayers=self.layers
   self.lock.unlock()

   for frame in 0..<Int(frames){
    var sum=0.0
    for index in localLayers.indices {
     var layer=localLayers[index]
     sum += self.sample(for:&layer,time:time,sampleRate:sampleRate) * layer.volume
     localLayers[index]=layer
    }
   ...
   self.lock.lock()
   if self.layers.count==localLayers.count {self.layers=localLayers}
   self.lock.unlock()
   return noErr
  }
 ...
 private func sample(for layer:inout Layer,time:Double,sampleRate:Double)->Double {
  let sine=sin(layer.phase)
  let random=Double.random(in:-1.0...1.0)
  let alpha=min(0.35,max(0.002,layer.band/sampleRate))
  layer.noiseState += alpha*(random-layer.noiseState)
  let pulse=max(0.2,layer.pulse)
  var value=0.0
  ...
  switch layer.type.lowercased() {
   ...
   case "brown":
    layer.noiseState=max(-1.0,min(1.0,layer.noiseState+Double.random(in:-0.04...0.04)))
    value=layer.noiseState
   case "hiss","widehiss","air","steam","static","crackle": value=random*0.72+layer.noiseState*0.28
   case "narrow": value=(random-layer.noiseState)*0.75+sin(layer.phase)*0.25
  ...
  layer.phase += 2.0*Double.pi*layer.frequency/sampleRate
  layer.phase2 += 2.0*Double.pi*(layer.frequency*1.018)/sampleRate
  if layer.phase > 2.0*Double.pi {layer.phase -= 2.0*Double.pi}
  if layer.phase2 > 2.0*Double.pi {layer.phase2 -= 2.0*Double.pi}
  return value
 }
