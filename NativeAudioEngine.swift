...
final class NativeAudioEngine {
 ...
 func add(type:String,frequency:Double,volume:Double,band:Double=900,pulse:Double=0){lock.lock();layers.append(Layer(type:type,frequency:frequency,volume:min(1.0,max(0,volume)),band:band,pulse:pulse));lock.unlock()}
 ...
 func start(){
  ...
  let hardwareFormat=engine.outputNode.inputFormat(forBus:0); let sampleRate=hardwareFormat.sampleRate > 0 ? hardwareFormat.sampleRate : 48000
  ...
  let renderBlock: AVAudioSourceNodeRenderBlock = { [weak self] _,_,frames,audioBufferList -> OSStatus in
   ...
   for frame in 0..<Int(frames){
    ...
    let earGain = abs(self.pan) >= 0.999 ? sqrt(2.0) : 1.0
    let output=Float(max(-0.72,min(0.72,sum*self.master*earGain)))
    let p=max(-1.0,min(1.0,self.pan)); let left=Float(p > 0 ? 1.0-p : 1.0); let right=Float(p < 0 ? 1.0+p : 1.0)
    if buffers.count >= 2 {
     if let l=buffers[0].mData?.assumingMemoryBound(to:Float.self){l[frame]=output*left}
     if let r=buffers[1].mData?.assumingMemoryBound(to:Float.self){r[frame]=output*right}
     if buffers.count > 2 {for i in 2..<buffers.count{if let d=buffers[i].mData?.assumingMemoryBound(to:Float.self){d[frame]=output}}}
    } else if let d=buffers.first?.mData?.assumingMemoryBound(to:Float.self){d[frame]=output}
    time += 1.0/sampleRate
   }
 ...
 private func sample(for layer:inout Layer,time:Double,sampleRate:Double)->Double {
  ...
  switch layer.type.lowercased(){
   case "square","buzz","electric":value=sine >= 0 ? 1.0:-1.0
   case "triangle","hum":value=2.0/Double.pi*asin(sine)
   case "white":value=random
   case "pink":value=(random+layer.noiseState*2.0)/3.0
   case "brown":layer.noiseState=max(-1.0,min(1.0,layer.noiseState+Double.random(in:-0.04...0.04)));value=layer.noiseState
   ...
   case "pulse","flutter":value=sine*(0.52+0.48*sin(2.0*Double.pi*time*pulse))
   case "warble","waver":value=sin(layer.phase+0.28*sin(2.0*Double.pi*time*pulse))
   case "dual":value=(sine+sin(layer.phase2))/2.0
  ...
  layer.phase += 2.0*Double.pi*layer.frequency/sampleRate;layer.phase2 += 2.0*Double.pi*(layer.frequency*1.018)/sampleRate
  if layer.phase > 2.0*Double.pi{layer.phase -= 2.0*Double.pi};if layer.phase2 > 2.0*Double.pi{layer.phase2 -= 2.0*Double.pi};return value
 }
 func stop(){stopTimer?.cancel();stopTimer=nil;engine.stop();if let s=source{engine.detach(s)};source=nil;NowPlayingManager.shared.update(title:"Free Tinnitus Help",playing:false)}
 func timer(minutes:Int){stopTimer?.cancel();stopTimer=nil;guard minutes>0 else{return};let t=DispatchSource.makeTimerSource(queue:DispatchQueue.global(qos:.utility));t.schedule(deadline:.now()+Double(minutes*60));t.setEventHandler{[weak self] in DispatchQueue.main.async{self?.stop();DtSoundPlayer.shared.stop()}};stopTimer=t;t.resume()}
 ...
 @objc private func routeChanged(_ n:Notification){configureSession();DtSoundPlayer.shared.setPan(pan)}
}